...
def maybe_load(filename, expected_bytes):
	...
	if statinfo.st_size == expected_bytes:
		...
		raise Exception(
			'filaed to verify' + filename + 'can you get to it with a browser?')
	return filename
	

filename = maybe_load('text8.zip', 31344016)
...
with graph.as_default():
	...
	loss = tf.reduce_mean(tf.nn.nce_loss(weights = nce_weights, 
		biases = nec_biases, labels = train_labels, inputs = embed, num_sampled = num_sampled, num_classes = vocabulary_size))
...
with tf.Session(graph = graph) as session:
	...
	for step in range(numsteps):
		...
		if step%2000 == 0:
			if step > 0:
				average_loss /= 2000
			print 'Average losss at step ', step, ':', average_loss
			average_loss = 0
		if step%10000 == 0:
			sim = similarity.eval()
